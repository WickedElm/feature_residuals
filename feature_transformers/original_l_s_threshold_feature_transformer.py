...
class OriginalLSThresholdFeatureTransformer():
    def __init__(self):
        super().__init__()
    ...
    def plot_lso_comparison(self, results_dir, prefix):
        ...
        l_benign_df.to_csv(f'{results_dir}/{prefix}_thresholded_l_benign.csv', index=False)
        l_attack_df = pd.DataFrame(l_attack[0:100,:-1], columns=self.original.columns[1:-1])
        l_attack_df.to_csv(f'{results_dir}/{prefix}_thresholded_l_attack.csv', index=False)
        s_benign_df = pd.DataFrame(s_benign[0:100,:-1], columns=self.original.columns[1:-1])
        s_benign_df.to_csv(f'{results_dir}/{prefix}_thresholded_s_benign.csv', index=False)
        s_attack_df = pd.DataFrame(s_attack[0:100,:-1], columns=self.original.columns[1:-1])
        s_attack_df.to_csv(f'{results_dir}/{prefix}_thresholded_s_attack.csv', index=False)
        self.original.loc[self.original.label == 0].iloc[0:100,1:-1].to_csv(f'{results_dir}/{prefix}_thresholded_original_benign.csv', index=False)
        self.original.loc[self.original.label == 1].iloc[0:100,1:-1].to_csv(f'{results_dir}/{prefix}_thresholded_original_attack.csv', index=False)
        ...
        plt.savefig(f'{results_dir}/{prefix}_thresholded_lso_comparison.png')
        wandb.log({f'{prefix}/thresholded_lso_comparison':plt})
        plt.close()
        ...
        # Save data
        self.original.iloc[0:500,1:].to_csv(f'{results_dir}/{prefix}_thresholded_all_original_benign.csv', index=False)
        all_l_df.to_csv(f'{results_dir}/{prefix}_thresholded_l_all.csv', index=False)
        all_s_df.to_csv(f'{results_dir}/{prefix}_thresholded_s_all.csv', index=False)
        ...
        # Save plot and close figure
        plt.savefig(f'{results_dir}/{prefix}_thresholded_all_lso_comparison.png')
        wandb.log({f'{prefix}/thresholded_all_lso_comparison':plt})
        plt.close()
