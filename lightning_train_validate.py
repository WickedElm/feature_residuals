...
class NetflowDataModule(pl.LightningDataModule):
    def __init__(self, data_path='./', batch_size=32, total_rows_threshold=500000, load_from_disk=False, prefix='', reserve_type=''):
        super().__init__()
...
class NetflowLogColumnsDataModule(pl.LightningDataModule):
    def __init__(self, data_path='./', batch_size=32, total_rows_threshold=500000, load_from_disk=False, prefix='', reserve_type=''):
        super().__init__()
        self.data_path = data_path
        self.batch_size = batch_size
    ...
    def setup(self, stage=None):
        ...
        ds.append_processor(dataset.DatasetProcessors.MakeTimestampColumnFirst(), process=False)
        ds.perform_processing()
        #ds.reserve_test_set(f'./tmp_ds_data/{ds.name}_test_reserve.pkl', split_point=0.15)
        ds.default_stratified_split(split_point_1=0.15, split_point_2=0.3)
        ds.save_indices()
        ds.write_indices_to_disk(opt.results_dir, f'{opt.save_prefix}_{opt.experiment}')
        ds.normalize_columns_min_max(target_cols=ds.cols_to_normalize)
...
class NetflowLogAllColumnsDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if not self.data_loaded:
            ...
            # Check for files to load here
            ds.perform_processing()

            # Reserve our data here (if it doesn't exist)
            # - Then put checks into the split

            # Reserve test set or load the previously saved test set from disk
            ds.reserve_test_set(f'./tmp_ds_data/{ds.name}_test_reserve.pkl', split_point=0.05)
            ...
            #new_df = pd.DataFrame(columns=ds.training_data.columns)
            #for i in range(5):
            #    new_df = new_df.append(ds.training_data.loc[ds.training_data.label == 0,:].iloc[0,:])

            #for i in range(5):
            #    new_df = new_df.append(ds.training_data.loc[ds.training_data.label == 1,:].iloc[0,:])

            #new_df = new_df.astype(ds.training_data.dtypes)

            #ds.training_data = copy.deepcopy(new_df)
            #ds.validation_data = copy.deepcopy(new_df)
            #ds.test_data = copy.deepcopy(new_df)

            # Construct training and validation sets
            self.ds_train = StandardNetflowDataset(ds.training_data, self.transform)
            self.ds_val = StandardNetflowDataset(ds.validation_data, self.transform)
            self.ds_test = StandardNetflowDataset(ds.test_data, self.transform)
            self.ds_test_extra = StandardNetflowDataset(ds.extra_test_data, self.transform)
...
class NetflowLogAllColumnsWeightedDataModule(pl.LightningDataModule):
    def __init__(self, data_path='./', batch_size=32, total_rows_threshold=500000, load_from_disk=False, prefix='', reserve_type=''):
        super().__init__()
        self.data_path = data_path
        self.batch_size = batch_size
...
class NetflowDropOHEColumnsDataModule(pl.LightningDataModule):
    def __init__(self, data_path='./', batch_size=32, total_rows_threshold=500000, load_from_disk=False, prefix='', reserve_type=''):
        super().__init__()
        self.data_path = data_path
        self.batch_size = batch_size
    ...
    def setup(self, stage=None):
        ...
        ds.append_processor(
            dataset.DatasetProcessors.DropColumns(
                target_cols=[
                    'protocol_tunneling',
                    'protocol_tcp',
                    'protocol_ip',
                    ...
                    'protocol_rtp',
                    'protocol_udp',
                    'communication_type_ftp',
                    'communication_type_ntp',
                    'communication_type_ldap',
                    'communication_type_dns',
                    'communication_type_ms-smb',
                    'communication_type_samba',
                    'communication_type_ssh',
                    'communication_type_smtps',
...
class NetflowStaticTestDataModule(pl.LightningDataModule):
    def __init__(self, data_path='./', batch_size=32, total_rows_threshold=500000, weighted=False, load_from_disk=False, prefix='', reserve_type=''):
        super().__init__()
        self.data_path = data_path
        self.batch_size = batch_size
...
class NetflowConferenceStandardizeNormalizationDataModule(pl.LightningDataModule):
    def __init__(self, data_path='./', batch_size=32, total_rows_threshold=500000, weighted=False, load_from_disk=False, prefix='', reserve_type=''):
        super().__init__()
        self.data_path = data_path
        self.batch_size = batch_size
    ...
    def setup(self, stage=None):
        ...
        if not self.data_loaded:
            ...
            ds.save_indices()
            ds.write_indices_to_disk(opt.results_dir, f'{opt.save_prefix}_{opt.experiment}')
            ds.normalize_columns(target_cols=ds.cols_to_normalize)
            ds.print_dataset_info()
...
class NetflowConferenceBenignOnlyTrainingAndValidationDataModule(pl.LightningDataModule):
    def __init__(self, data_path='./', batch_size=32, total_rows_threshold=500000, weighted=False, load_from_disk=False, prefix='', reserve_type=''):
        super().__init__()
        self.data_path = data_path
        self.batch_size = batch_size
    ...
    def setup(self, stage=None):
        ...
        if not self.data_loaded:
            ...
            # Only use benign samples for training and validation data for the RAE
            ds.training_data = ds.training_data.loc[ds.training_data.label == 0]
            ds.validation_data = ds.validation_data.loc[ds.validation_data.label == 0]
...
class NetflowConferenceAttackOnlyTrainingDataModule(pl.LightningDataModule):
    def __init__(self, data_path='./', batch_size=32, total_rows_threshold=500000, weighted=False, load_from_disk=False, prefix='', reserve_type=''):
        super().__init__()
        self.data_path = data_path
        self.batch_size = batch_size
    ...
    def setup(self, stage=None):
        ...
        if not self.data_loaded:
            ...
            # Only use attacks for  training the RAE
            ds.training_data = ds.training_data.loc[ds.training_data.label == 1]
...
class NetflowConferenceNoNormalizeDataModule(pl.LightningDataModule):
    def __init__(self, data_path='./', batch_size=32, total_rows_threshold=500000, weighted=False, load_from_disk=False, prefix='', reserve_type=''):
        super().__init__()
        self.data_path = data_path
        self.batch_size = batch_size
...
if __name__ == '__main__':
    ...
    # wandb init
    wandb.init(project=opt.project, name=f'{opt.experiment}-{ds_name}', group=opt.group)
    ...
    if opt.data_module == 'NetflowDataModule':
        ae = model_class(
            29,
            3,
            [25, 20, 15],
    ...
    elif opt.data_module == 'NetflowLogColumnsDataModule':
        ae = model_class(
            32,
            3,
            [25, 20, 15],
    ...
    elif opt.data_module == 'NetflowLogAllColumnsDataModule' or opt.data_module == 'NetflowLogAllColumnsWeightedDataModule' or opt.data_module == 'NetflowConferenceDataModule' or opt.data_module == 'NetflowConferenceNoNormalizeDataModule' or opt.data_module == 'NetflowConferenceAttackOnlyTrainingDataModule' or  opt.data_module == 'NetflowConferenceBenignOnlyTrainingDataModule' or opt.data_module == 'NetflowConferenceCICIDS2017BenignOnlyTrainingDataModule' or opt.data_module == 'NetflowStaticTestDataModule' or opt.data_module == 'NetflowConferenceStandardizeNormalizationDataModule' or opt.data_module == 'NetflowConferenceBenignOnlyTrainingAndValidationDataModule':

        if 'robust' in opt.model:
            ae = model_class(
                35,
                3,
                [28, 21, 15],
            ...
            ae = model_class(
                ...
                opt.lambda_filter,
                0.5,
                opt.hidden_layer_size
            )

    elif opt.data_module == 'NetflowDropOHEColumnsDataModule':
        ae = model_class(
            13,
            3,
            [11, 9, 7],
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=opt.results_dir, 
        save_top_k=-1, 
        every_n_val_epochs=50, 
        save_last=True, 
        filename=f'AE-{opt.project}-{opt.experiment}-{ds_name}' + '-{epoch:06d}'
    )

    model_callbacks = ae.get_callbacks(opt.num_epochs)
