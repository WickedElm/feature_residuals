...
class AutoencoderLinear3Bottleneck(pl.LightningModule):
    def __init__(self, in_dims, num_layers, layer_dims, results_dir='./', lr=1e-4, lambda_filter=0.01, dropout_rate=0.0, in_hidden_dim=15):
        ...
        layer_diff = math.ceil((in_dims - in_hidden_dim) / 3)

        layer1_out = in_dims - layer_diff
        layer2_out = layer1_out - layer_diff
        layer3_out = in_hidden_dim

        print(f'Layer Dims:  {in_dims}, {layer1_out}, {layer2_out}, {in_hidden_dim}')
        ...
        self.encoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(layer2_out, in_hidden_dim),
            nn.ReLU(),
        )

        self.decoder = nn.Sequential(
            nn.Linear(in_hidden_dim, layer2_out),
            nn.ReLU(),
