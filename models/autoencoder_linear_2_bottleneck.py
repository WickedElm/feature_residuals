...
class AutoencoderLinear2Bottleneck(pl.LightningModule):
    def __init__(self, in_dims, num_layers, layer_dims, results_dir='./', lr=1e-4, lambda_filter=0.01, dropout_rate=0.0, in_hidden_dim=15):
        ...
        layer_diff = math.ceil((in_dims - in_hidden_dim) / 2)

        layer1_out = in_dims - layer_diff
        layer2_out = in_hidden_dim
        ...
        self.encoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(layer1_out, in_hidden_dim),
            nn.ReLU(),
        )

        self.decoder = nn.Sequential(
            nn.Linear(in_hidden_dim, layer1_out),
            nn.ReLU(),
