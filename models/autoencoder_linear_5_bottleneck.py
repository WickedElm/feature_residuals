...
class AutoencoderLinear5Bottleneck(pl.LightningModule):
    def __init__(self, in_dims, num_layers, layer_dims, results_dir='./', lr=1e-4, lambda_filter=0.01, dropout_rate=0.0, in_hidden_dim=15):
        ...
        layer_diff = math.ceil((in_dims - in_hidden_dim) / 5)

        layer1_out = in_dims - layer_diff # 29
        layer2_out = layer1_out - layer_diff # 23
        layer3_out = layer2_out - layer_diff # 17
        layer4_out = layer3_out - layer_diff # 11
        layer5_out = in_hidden_dim # 7
        ...
        self.encoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(layer4_out, in_hidden_dim),
            nn.ReLU(),
        )

        self.decoder = nn.Sequential(
            nn.Linear(in_hidden_dim, layer4_out),
            nn.ReLU(),
